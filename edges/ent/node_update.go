...
func (nu *NodeUpdate) check() error {
	if v, ok := nu.mutation.Value(); ok {
		if err := node.ValueValidator(v); err != nil {
			return &ValidationError{Name: "value", err: fmt.Errorf(`ent: validator failed for field "Node.value": %w`, err)}
		}
	}
	return nil
}
